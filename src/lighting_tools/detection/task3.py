...
def render_scene(selected_on_ids, mode="L", halo=True):
    ...
    for fix in tmp:
        ...
        mask = np.array(Image.open(fix["mask"]).resize((x2 - x1, y2 - y1)).convert("L"))
    ...
    save_path = os.path.join(
        OUTDIR, f"render_on_{'_'.join(map(str,selected_on_ids))}.jpg"
    )
    img.save(save_path)
...
#     return img
# 예시: 1번, 2번 조명만 ON
img, patch = render_scene([1], mode="B", halo=True)
