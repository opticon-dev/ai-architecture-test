...
class CreatePartialDiff:
    def __init__(self, client, manifest):
        self.client = client
        self.manifest = manifest

    def process(self, fix, strength=0.3, guidance=7.0, seed=1234):
        ...
        out_on = self.client.run("black-forest-labs/flux-kontext-dev", input=payload_on)
        out_off = self.client.run(
            "black-forest-labs/flux-kontext-dev", input=payload_off
        )
